...
def chunk_runner(fun, data):
    ret = []
    for datum in data: # next(data): data: __iter__
        ret.append(fun(datum))
    return ret


def chunked_async_map(pool, mapper, data, chunk_size) -> List[AsyncResult]:
    async_returns = []
...
if __name__ == '__main__':
    words = [
        word for word in map(lambda x: x.strip().rstrip(),
    ...
    tp_start: float = time.perf_counter()
    counts = map_reduce(pool, words, emitter, counter, chunk_size, reporter)
    pool.close()
    pool.join()
    tp_end: float = time.perf_counter()
    
    
    for count in sorted(counts, key=lambda x: x[1]):
        print(count)

    print(f"[Elapsed time mp process]: {(tp_end - tp_start) * 1000:.2f}[ms]")
